...
class JavaClassUseSiteMemberScope(
    ...
    fun myScope(className: String, methodName: String, name: Name): Boolean {
        val fullClassName = if ('/' !in className) "/$className" else className
        return this.toString() == "Java use site scope of $fullClassName" && name.asString() == methodName
    }

    fun shouldStop(name: Name) = myScope("B", "setScope", name)
